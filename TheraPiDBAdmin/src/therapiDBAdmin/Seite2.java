...
public class Seite2 extends JXPanel {
                ...
                if (checkalt.isSelected()) {
                    ...
                    if (!testDBCreate()) {
                        JOptionPane.showMessageDialog(null,
                                "Die Anlage der Datenbank und/oder neuem Benutzer ist fehlgeschlagen");
                        return;
                    } else {
                        JOptionPane.showMessageDialog(null,
                                "Datenbank und Datenbank-Benutzer wurden erfolgreich angelegt");
                        eltern.setSeite2Ok(true);
                        eltern.sqlTab.setSelectedIndex(2);
                        buts[0].setEnabled(false);
                        doTestConnectivity();
    ...
    private boolean testDBCreate() {
        ...
        if (!String.valueOf(pw1.getPassword())
                   .equals(tfs[2].getText()
                                 .trim())) {
        ...
        try {
            ...
            stmt.executeUpdate("CREATE DATABASE " + tfs[0].getText()
                                                          .trim());
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(null, ex.getMessage());
            JOptionPane.showMessageDialog(null,
                    "Anlegen der Datenbank fehlgeschlagen, vermutlich existiert die DB bereits!");
            // JOptionPane.showMessageDialog(null,ex.getMessage());
            return false;
        }
        try {
            stmt.executeUpdate("GRANT ALL ON " + tfs[0].getText()
                                                       .trim()
                    + ".* TO '" + tfs[1].getText()
                                        .trim()
                    + "'@'%' IDENTIFIED BY '" + tfs[2].getText()
                                                      .trim()
                    + "'");
            stmt.executeUpdate("GRANT ALL ON " + tfs[0].getText()
                                                       .trim()
                    + ".* TO '" + tfs[1].getText()
                                        .trim()
                    + "'@'localhost' IDENTIFIED BY '" + tfs[2].getText()
                                                              .trim()
                    + "'");
            eltern.setParams(tfs[0].getText()
                                   .trim(),
                    tfs[1].getText()
                          .trim(),
                    tfs[2].getText()
            ...
            JOptionPane.showMessageDialog(null, ex.getMessage());
            JOptionPane.showMessageDialog(null,
                    "Anlegen des neuen Benutzers fehlgeschlagen, vermutlich existiert der Benutzer bereits!");
            // JOptionPane.showMessageDialog(null,ex.getMessage());
            return false;
        }
        return bret;
