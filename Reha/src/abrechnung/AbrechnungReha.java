...
public class AbrechnungReha extends JXPanel {
            ...
            protected Void doInBackground() throws Exception {
                try {
                    ...
                    if ((patient != null) && (Reha.instance.patpanel.vecaktrez.size() > 0)) {
                        if (Reha.instance.patpanel.vecaktrez.get(1)
                                                            .startsWith("RH")) {
    ...
    private JXPanel getButtons() {
        FormLayout lay = new FormLayout(
                // 1 2 3 4 5 6 7 8
                "fill:0:grow(0.5),80dlu,5dlu,80dlu,5dlu,80dlu,fill:0:grow(0.5)", "10dlu,p,10dlu");
    ...
    private void doRechnen() {
        ...
        for (int i = 0; i < 4; i++) {
            zeilenPreis[i] = BigDecimal.valueOf(Double.parseDouble(tfanzahl[i].getText()
                                                                              .trim()
                    + ".00"))
                                       .multiply(BigDecimal.valueOf(Double.parseDouble(tfpreis[i].getText()
                                                                                                 .replace(",", "."))));
            tfgesamt[i].setText(dcf.format(zeilenPreis[i].doubleValue())
                                   .replace(".", ","));
    ...
    private void doRechnenPat() {
        gesamtPatPreis = BigDecimal.valueOf(Double.parseDouble(tfpatanzahl[0].getText()
                                                                             .trim()
                + ".00"))
                                   .multiply(BigDecimal.valueOf(Double.parseDouble(tfpatpreis[0].getText()
                                                                                                .replace(",", "."))));
    ...
    private void doEinlesen(String rehanummer) {
        ...
        new SwingWorker<Void, Void>() {
            @Override
            protected Void doInBackground() throws Exception {
                holePatientUndKostentraeger(rehavec.get(0)
                                                   .get(0)
                                                   .trim(),
                        rehavec.get(0)
                               .get(37)
                               .trim());
                holeRechnungsAnschrift(rehavec.get(0)
                                              .get(37)
                                              .trim());
                return null;
            }
        ...
        int preisgruppe = Integer.parseInt(rehavec.get(0)
                                                  .get(41));
        String artdbeh;
        for (int i = 0; i < 4; i++) {
            jcmb[i].setDataVectorWithStartElement(SystemPreislisten.hmPreise.get("Reha")
                                                                            .get(preisgruppe - 1),
                    0, 9, "./.");
            artdbeh = rehavec.get(0)
                             .get(8 + i);
            if (!artdbeh.equals("0")) {
                tfanzahl[i].setText(rehavec.get(0)
                                           .get(3 + i));
        ...
        jcmbpat[0].setDataVectorWithStartElement(SystemPreislisten.hmPreise.get("Reha")
                                                                           .get(preisgruppe - 1),
                0, 9, "./.");
        new SwingWorker<Void, Void>() {
            @Override
            protected Void doInBackground() throws Exception {
                try {
                    tagevec = RezTools.holeEinzelTermineAusRezept("", rehavec.get(0)
                                                                             .get(34));
                    if (Integer.parseInt(tfanzahl[0].getText()
                                                    .trim()) != tagevec.size()) {
                        String meldung = "Achtung!\nDie Anzahl der erfaßten Einzeltermine stimmt nicht mit der\n"
                                + "Anzahl der Verordnung überein!\n\n" + "Anzahl der erfaßten Einzeltermine = "
                                + Integer.toString(tagevec.size()) + "\n" + "Anzahl laut Angaben auf der Verordnung = "
                                + tfanzahl[0].getText()
                                             .trim()
    ...
    private void holePatientUndKostentraeger(String pat_intern, String kassenid) {
        patvec = SqlInfo.holeFelder(
                "select n_name,v_name,geboren,anrede,titel,strasse,plz,ort,kv_status from pat5 where pat_intern='"
                        + pat_intern + "' LIMIT 1");
        kassvec = SqlInfo.holeFelder(
                "select kassen_nam1,kassen_nam2,strasse,plz,ort,ik_papier,ik_kostent from kass_adr where id='"
                        + kassenid + "' LIMIT 1");
        rechnungvec = (Vector<Vector<String>>) kassvec.clone();
        anschriftID = rehavec.get(0)
                             .get(37)
                             .trim()
                             .trim();
        if (patvec.size() <= 0) {
        ...
        istprivat = kassvec.get(0)
                           .get(4)
                           .trim()
                           .equals("");
        patLabel.setText(StringTools.EGross(patvec.get(0)
                                                  .get(0))
                + ", " + StringTools.EGross(patvec.get(0)
                                                  .get(1))
    ...
    private void doKassenLabel() {
        kassenLabel.setText(rechnungvec.get(0)
                                       .get(0)
                + " " + rechnungvec.get(0)
                                   .get(1)
                + " " + rechnungvec.get(0)
    ...
    private void holeRechnungsAnschrift(String id) {
        rechnungvec = SqlInfo.holeFelder(
                "select kassen_nam1,kassen_nam2,strasse,plz,ort,ik_papier,ik_kostent from kass_adr where id='" + id
                        + "' LIMIT 1");
        doKassenLabel();
    }
    ...
    private void doAnschrift() {
        ...
        if (!tfs[0].getText()
            ...
            if (!tfs[2].getText()
                       .trim()
                       .equals(anschriftID)) {
                anschriftID = String.valueOf(tfs[2].getText()
                                                   .trim());
                    ...
                    if (abrechnungOk) {
                        ...
                        if (!abrechnungOk) {
                            JOptionPane.showConfirmDialog(null,
                                    "Während der Rechnungserstellung ist ein Fehler aufgetreten!");
                        } else {
                            if (Reha.vollbetrieb) {
                                doUebertragen();
                            }
                        ...
                        JOptionPane.showConfirmDialog(null,
                                "Die Abrechnung enthält Fehler. Bitte beheben Sie die Fehler und starten Sie die RehaAbrechnung erneut");
                        allesAusschalten();
                    }
    ...
    private boolean do301er() {
        try {
            ...
            for (int i = 0; i < 4; i++) {
                if (tfanzahl[i].getText()
                               .trim()
                               .equals("0")) {
                    break;
                } else {
                    obj[0] = tfanzahl[i].getText()
                                        .trim();
                    obj[1] = tfpreis[i].getText()
                                       .trim();
                    obj[2] = jcmb[i].getValueAt(2)
                                    .toString()
                                    .trim();
            ...
            if (objvec.size() <= 0) {
                JOptionPane.showMessageDialog(null,
                        "Keine Positionen angegeben, keine Abrechnung möglich - ganz einfach!");
                return false;
            }
            String id = SqlInfo.holeEinzelFeld("select id from dta301 where pat_intern='" + rehavec.get(0)
                                                                                                   .get(0)
                    + "' and rez_nr='" + rehavec.get(0)
                                                .get(1)
                    + "' and nachrichtentyp='1' LIMIT 1");
            if (id.equals("")) {
                JOptionPane.showMessageDialog(null,
    ...
    private void doUebertragen() {
        String delrez = String.valueOf(rehavec.get(0)
                                              .get(1));
        ...
        if (aktiverPatient.equals(rehavec.get(0)
                                         .get(0))) {
            posteAktualisierung(rehavec.get(0)
                                       .get(0));
        }
    }
    ...
    private void doEigenanteilDrucken() throws Exception {
        ...
        istprivat = true;
        hmRechnung.clear();
        String[] padressDaten = PatTools.constructPatHMapFromStrings(patvec.get(0)
                                                                           .get(3),
                patvec.get(0)
                      .get(4),
                patvec.get(0)
        ...
        hmRechnung.put("<pri7>", StringTools.EGross(patvec.get(0)
                                                          .get(0))
                + ", " + StringTools.EGross(patvec.get(0)
                                                  .get(1))
                + ", geb.am: " + DatFunk.sDatInDeutsch(patvec.get(0)
        ...
        vecpos.add(jcmbpat[0].getSelectedItem()
                             .toString());
        vecpos.add(tfpatanzahl[0].getText()
                                 .trim());
        vecpos.add(tfpatpreis[0].getText()
                                .trim());
        vecpos.add(tfpatgesamt[0].getText()
                                 .trim());
        vecpos.add(jcmbpat[0].getValueAt(2)
                             .toString());
        vecpos.add(jcmbpat[0].getValueAt(9)
                             .toString());
        vecposrechnung.add((Vector<String>) vecpos.clone());
        gesamtPreis = BigDecimal.valueOf(Double.parseDouble(tfpatgesamt[0].getText()
                                                                          .trim()
                                                                          .replace(",", ".")));
    ...
    private void doFaktura() {
        ...
        for (int i = 0; i < vecposrechnung.size(); i++) {
            ...
            if (i == 0) {
                ...
                try {
                    if (hmRechnung.get("<pri4>")
                                  .indexOf(" ") >= 0) {
                        rechnungBuf.append("plz='" + hmRechnung.get("<pri4>")
                                                               .split(" ")[0]
                                + "', ");
                        int leeran = hmRechnung.get("<pri4>")
                                               .indexOf(" ");
                    ...
                    rechnungBuf.append("name='" + patvec.get(0)
                                                        .get(0)
                            + "," + patvec.get(0)
                                          .get(1)
                            + ",geb." + DatFunk.sDatInDeutsch(patvec.get(0)
                                                                    .get(2))
                            + "', ");
                } catch (Exception ex) {
            ...
            rechnungBuf.append("status='" + patvec.get(0)
                                                  .get(8)
                    + "', ");
            rechnungBuf.append("pos_kas='" + vecposrechnung.get(i)
                                                           .get(4)
                    + "', ");
            rechnungBuf.append("pos_int='" + vecposrechnung.get(i)
                                                           .get(5)
                    + "', ");
            rechnungBuf.append("anzahl='" + vecposrechnung.get(i)
                                                          .get(1)
                    + "', ");
            rechnungBuf.append("anzahltage='" + vecposrechnung.get(i)
                                                              .get(1)
                    + "', ");
            rechnungBuf.append("preis='" + vecposrechnung.get(i)
                                                         .get(2)
                                                         .replace(",", ".")
                    + "', ");
            rechnungBuf.append("gesamt='" + vecposrechnung.get(i)
                                                          .get(3)
                                                          .replace(",", ".")
                    + "', ");
            rechnungBuf.append("zzbetrag='" + "0.00" + "', ");
            rechnungBuf.append("netto='" + vecposrechnung.get(i)
                                                         .get(3)
                                                         .replace(",", ".")
                    + "', ");
            rechnungBuf.append("pauschale='" + "0.00" + "', ");
            rechnungBuf.append("rez_nr='" + rehavec.get(0)
                                                   .get(1)
                    + "', ");
            rechnungBuf.append("rezeptart='8', ");
            rechnungBuf.append("pat_intern='" + rehavec.get(0)
                                                       .get(0)
                    + "', ");
            rechnungBuf.append("rnummer='" + aktRechnung + "', ");
            rechnungBuf.append("kassid='" + rehavec.get(0)
                                                   .get(37)
                    + "', ");
            rechnungBuf.append("arztid='" + rehavec.get(0)
                                                   .get(16)
                    + "', ");
    ...
    private void doAnlegenOP() {
        ...
        rechnungBuf.append("r_name='" + patvec.get(0)
                                              .get(0)
                + "," + patvec.get(0)
                              .get(1)
                + "," + DatFunk.sDatInDeutsch(patvec.get(0)
                                                    .get(2))
                + "', ");
        rechnungBuf.append("r_klasse='" + "RH" + "', ");
        rechnungBuf.append("r_betrag='" + dcf.format(gesamtPreis.doubleValue())
                                             .replace(",", ".")
                + "', ");
        rechnungBuf.append("r_offen='" + dcf.format(gesamtPreis.doubleValue())
                                            .replace(",", ".")
                + "', ");
        ...
        try {
            if (istprivat) {
                ...
                hmRechnung.put("<pri2>", rechnungvec.get(0)
                                                    .get(1));
                hmRechnung.put("<pri3>", rechnungvec.get(0)
                                                    .get(2));
                hmRechnung.put("<pri4>", rechnungvec.get(0)
                                                    .get(3)
                        + " " + rechnungvec.get(0)
                                           .get(4));
            ...
            if (aktRechnung.equals("-1")) {
                JOptionPane.showMessageDialog(null,
                        "Fehler - Rechnungsnummer für Reharechnung kann nicht bezogen werden");
                abrechnungOk = false;
                return;
            }
            hmRechnung.put("<pri6>", aktRechnung);
            ...
            if (jcmb[0].getSelectedItem()
                ...
                hmRechnung.put("<pri9>", "nicht relevant");
            } else if (jcmb[0].getSelectedItem()
                              .toString()
                              .toUpperCase()
                              .startsWith("LVA")
                    || jcmb[0].getSelectedItem()
                              .toString()
                              .toUpperCase()
                              .startsWith("BFA")
                    || jcmb[0].getSelectedItem()
                ...
                if (vsnr.trim()
                        .length() == 12) {
                    vsnr = vsnr.substring(0, 2) + " " + vsnr.substring(2, 8) + " " + vsnr.substring(8, 9) + " "
                            + vsnr.substring(9);
                }
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (jcmb[i].getSelectedIndex() > 0) {
                    vecpos.add(jcmb[i].getSelectedItem()
                                      .toString());
                    vecpos.add(tfanzahl[i].getText()
                                          .trim());
                    vecpos.add(tfpreis[i].getText()
                                         .trim());
                    vecpos.add(tfgesamt[i].getText()
                                          .trim());
                    if (jcmb[i].getSelectedIndex() > 0) {
                        vecpos.add(jcmb[i].getValueAt(2)
                                          .toString());
                        vecpos.add(jcmb[i].getValueAt(9)
                                          .toString());
                    } else {
                        vecpos.add("");
                        vecpos.add("");
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(null,
                    "In der Abrechnung ist ein Fehler aufgetreten, bitte beheben Sie den Fehler und starten Sie die Abrechnung erneut");
            abrechnungOk = false;
        }
    }
            ...
            if (placeholders[i].getDisplayText()
                ...
                placeholders[i].getTextRange()
                               .setText(hmAdresse.get("<pri9>"));
                                      ...
                                      .equals("<pri10>")) {
                placeholders[i].getTextRange()
                               .setText(hmAdresse.get("<pri10>"));
            } else if (placeholders[i].getDisplayText()
                                      .toLowerCase()
                                      .equals("<pri11>")) {
                placeholders[i].getTextRange()
                               .setText(hmAdresse.get("<pri11>"));
            }
        }
    ...
    private void startePositionen(Vector<Vector<String>> vecpositionen, BigDecimal gesamt) throws TextException {
        ...
        textEndbetrag.getCell(1, 0)
                     .getTextService()
                     .getText()
                     .setText(dcf.format(gesamt.doubleValue()) + " EUR");
    }
