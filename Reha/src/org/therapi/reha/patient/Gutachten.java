...
public class Gutachten extends JXPanel implements ActionListener, TableModelListener, PropertyChangeListener {
            ...
            public void run() {
                new SwingWorker<Void, Void>() {
                    ...
                    protected Void doInBackground() throws Exception {
                        ...
                        FormLayout vplay = new FormLayout("5dlu,fill:0:grow(1.00),5dlu",
                                "13dlu,fill:0:grow(1.00),5dlu");
    ...
    private JXPanel getGutachtenTbl() {
        ...
        /*
         * 0 "berichtid," + 1 "bertitel," + 2 "verfasser," + 3
         * "DATE_FORMAT(erstelldat,'%d.%m.%Y') AS derstelldat," + 4 "empfaenger," + 5
         * "DATE_FORMAT(editdat,'%d.%m.%Y') AS deditdat, 6 "empfid",
         */
        tabbericht.getColumn(0)
                  .setMinWidth(50);
        tabbericht.getColumn(0)
                  .setMaxWidth(50);
        tabbericht.getColumn(1)
                  .setMinWidth(140);
        tabbericht.getColumn(3)
                  .setMaxWidth(80);
        tabbericht.getColumn(5)
                  .setMaxWidth(80);
        tabbericht.getColumn(6)
                  .setMinWidth(0);
        tabbericht.getColumn(6)
                  .setMaxWidth(0);
        tabbericht.getColumn(7)
                  .setMinWidth(0);
        tabbericht.getColumn(7)
                  .setMaxWidth(0);
        tabbericht.validate();
...
//	Gutachten.gutachten.aktualisiereGutachten(datFunk.sHeute(),(empf.contains("DRV") ? "DRV E-Bericht" : "GKV E-Bericht"),empf,"Rehaarzt",berichtid);
    ...
    public void aktualisiereGutachten(String editdat, String bertype, String empf, String verfasser, int berid,
            String patintern) {
        ...
        if (!Reha.instance.patpanel.aktPatID.equals(patintern)) {
            JOptionPane.showMessageDialog(null,
                    "Der aktuelle Patient und das zu speichernde Gutachten passen nicht zusammen...");
            return;
        }
        if (dtblm.getValueAt(row, 0)
                 .equals(Integer.toString(berid))) {
    ...
    public void neuesGutachten(String berid, String bertype, String verfasser, String erstellt, String empfang,
            String patintern, String bertitel) {
            ...
            protected Void doInBackground() throws Exception {
                try {
                    ...
                    Vector<Vector<String>> vec = SqlInfo.holeSaetze("berhist",
                            "berichtid," + "berichttyp," + "verfasser,"
                                    + "DATE_FORMAT(erstelldat,'%d.%m.%Y') AS derstelldat," + "empfaenger,"
                                    + "DATE_FORMAT(editdat,'%d.%m.%Y') AS deditdat," + "empfid," + "bertitel",
                    ...
                    for (int i = 0; i < anz; i++) {
                        ...
                        if (((String) ((Vector<?>) vec.get(i)).get(2)).toUpperCase()
                                                                      .contains("REHAARZT")
                                || ((String) ((Vector<?>) vec.get(i)).get(2)).toUpperCase()
                                                                             .contains("REHA-ARZT")) {
                    ...
                    try {
                        SwingUtilities.invokeLater(new Runnable() {
                            ...
                            public void run() {
                                Reha.instance.patpanel.getTab()
                                                      .setTitleAt(4, macheHtmlTitel(fanz, "Gutachten"));
                            }
                        });
            ...
            new SwingWorker<Void, Void>() {
                ...
                protected Void doInBackground() throws Exception {
                    try {
                        ...
                        GutachtenWahl gwahl = new GutachtenWahl(comp.getLocationOnScreen(), tf,
                        ...
                        if (tf.getText()
                              .equalsIgnoreCase("ebericht")) {
                            Reha.instance.progLoader.GutachenFenster(1, Reha.instance.patpanel.aktPatID, -1,
                                    "E-Bericht", true, "", -1);
                            // ProgLoader.GutachenFenster(1,Reha.instance.patpanel.aktPatID
                            // ,-1,"E-Bericht",true,"" );
                            return null;
                        }
                        if (tf.getText()
                              .equalsIgnoreCase("nachsorge")) {
                            Reha.instance.progLoader.GutachenFenster(1, Reha.instance.patpanel.aktPatID, -1,
                                    "Nachsorge", true, "", -1);
                            // ProgLoader.GutachenFenster(1,Reha.instance.patpanel.aktPatID
                            // ,-1,"Nachsorge",true,"" );
    ...
    private void doBerichtEdit() {
        ...
        int berid = Integer.parseInt(tabbericht.getValueAt(row, 0)
                                               .toString());
        String berempfaenger = (String) tabbericht.getValueAt(row, 4);
        Reha.instance.progLoader.GutachenFenster(1, Reha.instance.patpanel.aktPatID, berid, bertyp, false,
                berempfaenger, -1);
        // ProgLoader.GutachenFenster(1,Reha.instance.patpanel.aktPatID
        // ,berid,bertyp,false,berempfaenger );
    ...
    class MyGutachtenTableModel extends DefaultTableModel {
        ...
        */
        ...
        public boolean isCellEditable(int row, int col) {
            ...
            return false;
            /*
             * if (col == 0){ return true; }else if(col == 3){ return true; }else if(col ==
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                try {
                    ...
                    pt.x = pt.x + 150;
                    GutachtenWahl gwahl = new GutachtenWahl(pt, tf,
                            "<html><font color='#ffff66'>Stammdaten auf neues Gutachten übertragen</font></html>");
                    // System.out.println("Aufruf des Focus***********");
                    gwahl.setzeFocus();
                    gwahl.setVisible(true);
                    // gwahl.setModal(true);
                    ...
                    if (tf.getText()
                        ...
                        int uebernahme = Integer.parseInt(tabbericht.getValueAt(row, 0)
                                                                    .toString());
                        Reha.instance.progLoader.GutachenFenster(1, Reha.instance.patpanel.aktPatID, -1, "E-Bericht",
                                true, "", uebernahme);
                        return null;
                    }
                    if (tf.getText()
                        ...
                        int uebernahme = Integer.parseInt(tabbericht.getValueAt(row, 0)
                                                                    .toString());
                        Reha.instance.progLoader.GutachenFenster(1, Reha.instance.patpanel.aktPatID, -1, "Nachsorge",
                                true, "", uebernahme);
                        return null;
                    }
                    gwahl = null;
    ...
    class ToolsDlgGutachten {
        public ToolsDlgGutachten(String command, Point pt) {
            ...
            JList list = new JList(new Object[] { "Stammdaten in neues Gutachten übertragen",
                    "ASP-Patienten-Information", "ASP-Verordnung", "ASP-Anwesenheitsliste",
                    "ASP-Zustimmung des Patienten", "IRENA-Patienten-Information", "IRENA-Verordnung",
                    "IRENA-Anwesenheitsliste", "IRENA-Zustimmung des Patienten", "IRENA-Beginnmitteilung",
                    "BfA - Reha-Verlängerung", "DRV-Wiedereingliederung Pat./AG-Infos", "DRV-Wiedereingliederung",
                    "DRV-Wiedereingliederung Beginn > 2 Wo.", "BfA - AHB-Aufnahmemitteilung", "ASP-Rechnungsformular",
                    "IRENA-Rechnungsformular", "Reha-Anwesenheitsbescheinigung", "GKV-Verlängerungsantrag",
