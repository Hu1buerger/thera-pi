...
public class Dta301 extends JXPanel implements FocusListener {
        ...
        al = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (cmd.equals("original")) {
                    ...
                    String scmd = "select nachrichtorg from dtafall where id ='" + tabuebersicht.getValueAt(row, 3)
                            + "' LIMIT 1";
                ...
                if (cmd.equals("aufbereitet")) {
                    ...
                    String id = tabuebersicht.getValueAt(row, 3)
                                             .toString();
                    String typ = (tabuebersicht.getValueAt(row, 0)
                                               .toString()
                                               .contains("Ablehnung") ? "2" : "1");
                    Vector<Vector<String>> vec = SqlInfo.holeFelder(
                            "select pat_intern,rez_nr from dtafall where id ='" + id + "' LIMIT 1");
                    String scmd = "select edifact from dta301 where pat_intern='" + vec.get(0)
                                                                                       .get(0)
                            + "' and rez_nr='" + vec.get(0)
                                                    .get(1)
                            + "' and nachrichtentyp='" + typ + "' LIMIT 1";
                    doZeigeEdifact(SqlInfo.holeEinzelFeld(scmd));
                }
                if (cmd.equals("deletemessage")) {
                    ...
                    int frage = JOptionPane.showConfirmDialog(null,
                            "Die Nachricht mit ID = " + id + " wirklich löschen?", "Achtung wichtige Benutzeranfrage",
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getSource() instanceof JXTable) {
                    if (e.getClickCount() == 1 && e.getButton() == 3) {
                        ...
                        String typ = tabuebersicht.getValueAt(row, 0)
                                                  .toString();
                        boolean bewilligung = tabuebersicht.getValueAt(row, 0)
                                                           .toString()
                                                           .equals("Bewilligung");
    ...
    private JXPanel getVerlaengerung() {
        ...
        /*
         * vcombo = new JRtaComboBox(); String[][] codeListe =
         * Dta301CodeListen.getCodeListe("B08"); for(int i = 0; i <
         * codeListe.length;i++){ ucombo.addItem(codeListe[i][1]); }
         * vcombo.setSelectedIndex(2); pan.add(vcombo,cc.xyw(3,12,3));
         */
    ...
    private JXPanel getEntlassung() {
        ...
        // Tagestabelle bauen
        /*
         * modentlass = new MyTermTableModel(); String[] column =
         * {"Beh.Datum","Behandler","Text","Beh.Art",""};
         * modentlass.setColumnIdentifiers(column); tabentlass = new
         * JXTable(modentlass);
         */
    ...
    private JXPanel getUebersicht() {
        ...
        tabuebersicht.getColumn(0)
                     .setMinWidth(150);
        tabuebersicht.getColumn(3)
                     .setMinWidth(0);
        tabuebersicht.getColumn(3)
                     .setMaxWidth(35);
        tabuebersicht.getColumn(4)
                     .setMaxWidth(60);
        tabuebersicht.getColumn(5)
                     .setMaxWidth(60);
    ...
    private void holeDaten() {
        ...
        try {
            ...
            buf.append(
                    "TD{font-family: Arial; font-size: 12pt; padding-left:5px;padding-right:5px;padding-top:0px,padding-bottom:0px}");
            ...
            buf.append("<table>");
            /****************************/
            buf.append("<tr><td class=\"spalte3\" align=\"left\">" + "<img src='file:///" + Path.Instance.getProghome()
                    + "icons/rezept.png' border=0>");
            buf.append("</td></tr>");
            buf.append("<tr><td class=\"spalte1\"><font size=\"+1\"><b>" + reznummer);
            buf.append("</b></font></td></tr>");
            buf.append("<tr><td>&nbsp</td></tr>");
            buf.append("<tr><td class=\"spalte1\" align=\"left\">" + "<img src='file:///" + Path.Instance.getProghome()
                    + "icons/kontact_contacts.png' width=52 height=52 border=0>");
            buf.append("</td></tr>");
            ...
            if (Integer.parseInt(SqlInfo.holeEinzelFeld("select count(*) from dta301 where pat_intern='" + patnummer
                ...
                is301Ok = false;
                buf.append("<tr><td class=\"spalte1\"><font size=\"+1\" color=#ff0000><b>"
                        + "§301-DTA nicht möglich mit dieser Verordnung");
                buf.append("</b></font></td></tr>");
            } else {
                String cmd = "select kassen_nam1 from kass_adr where ik_kasse ="
                        + "(select ktraeger from dta301 where pat_intern='" + patnummer + "' and rez_nr='" + reznummer
                        + "' LIMIT 1) LIMIT 1";
    ...
    private void doTabelleFuellen() {
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                try {
                    String[] anlass = { "Unbekannter Anlass", "Bewilligung", "Ablehnung", "Aufnahmemitteilung",
                            "Unterbrechung (Beginn)", "Verlängerung", "Entlassmitteilung", "E-Bericht", "Rechnung",
                            "Absage an den Kostenträger", "Einberufung", "Rückstellung",
                    ...
                    for (int i = 0; i < vec.size(); i++) {
                        ...
                        dummy.add(DatFunk.sDatInDeutsch(vec.get(i)
                                                           .get(1)));
                        dummy.add(vec.get(i)
                                     .get(2));
                        dummy.add(vec.get(i)
    ...
    private JXHeader getHeader(int welcher) {
        ...
        case 0:
            ...
            head.setDescription(
                    "<html>Tragen Sie hier bitte das <u>Datum der Eingangsuntersuchung</u> des Patienten sowie die <u>Uhrzeit</u> der Aufnahme ein.<br>"
                            + "Im Fall einer Absage benutzen Sie bitte die untere Rubrik</html>");
            head.setIcon(new ImageIcon(Path.Instance.getProghome() + "icons/start.jpg"));
            break;
        case 1:
            head.setTitle("Unterbrechung eine Reha");
            head.setDescription(
                    "<html><br>Hier melden Sie <u>Reha-Unterbrechungen</u> sowie den <u>Grund der Unterbrechung</u> (z.B.Krankheit.)<br>"
            ...
            head.setIcon(new ImageIcon(Path.Instance.getProghome() + "icons/break.jpg"));
            break;
        case 2:
            head.setTitle("Entlass-Mitteilung eines Reha-Patienten");
            head.setDescription(
                    "<html><br><u>Kontrollieren Sie im Rezeptstamm die einzelnen Datumswerte</u> auf Richtigkeit.<br><br>Tragen Sie dann das <u>Datum der Aufnahme</u> und das <u>Datum der Entlassung</u> ein.</html>");
            head.setIcon(new ImageIcon(Path.Instance.getProghome() + "icons/ziel.jpg"));
            break;
        case 3:
            head.setTitle("Übersicht über alle Meldungen die diesen Fall betreffen");
            head.setDescription(
                    "<html><br>Datmit eine lückenlose Fall-Kontrolle möglich ist können Sie sich hier<br>einen Überblick über die gesamte Kommunikation mit dem jeweiligen Kostenträger verschaffen.</html>");
            head.setIcon(new ImageIcon(Path.Instance.getProghome() + "icons/uebersicht.jpg"));
            break;
    ...
    private boolean doRegle301(int art, String id) {
        if (art == 0) {
            ...
            meldung301.doBeginn(this.beginndatum.getText()
                                                .trim(),
                    this.beginnstunde.getText()
                                     .trim()
                            + this.beginnminute.getText()
                                               .trim(),
                    beginneditpan.getText()
                                 .trim(),
                    aufnahmeart, beginncheck.isSelected(), (this.beginnahbcheck.isSelected()
                            ? beginnahbentlass.getText()
                                              .trim()
                            : null));
            doTabelleFuellen();
            return true;
        ...
        if (art == 1) {
            ...
            RVMeldung301 meldung301 = new RVMeldung301(art, id,
                    Integer.parseInt(SqlInfo.holeEinzelFeld(
                            "select count(*) from dtafall where nachrichtorg like '%PRC+ADMIN6%' AND rez_nr='"
                                    + this.reznummer + "'"))
                            + 1);
            int iart = -1;
            for (int i = 0; i < 3; i++) {
            ...
            meldung301.doUnterbrechung(this.ubbeginndatum.getText()
                                                         .trim(),
                    this.ubendedatum.getText()
                                    .trim(),
                    iart, ucombo.getSelectedIndex(), this.ueditpan.getText()
                                                                  .trim());
            doTabelleFuellen();
            return true;
        }
        if (art == 2) {
            ...
            RVMeldung301 meldung301 = new RVMeldung301(art, id,
                    Integer.parseInt(SqlInfo.holeEinzelFeld(
                            "select count(*) from dtafall where nachrichtorg like '%PRC+ETG%' AND rez_nr='"
                                    + this.reznummer + "'"))
                            + 1);
            // JRtaCheckBox entlassmitfahrgeld = null;
            // JRtaTextField entlassfahrgeld = null;
            // JTextArea entlasstpan = null;
            if (DatFunk.TageDifferenz(this.entlasserstdatum.getText()
                                                           .trim(),
                    this.entlassletztdatum.getText()
                                          .trim()) < 0) {
                JOptionPane.showMessageDialog(null, "Entlassungsdatum ist vor dem Aufnahmetag!!!\n\nBitte überprüfen");
                return false;
            }
            meldung301.doEntlassung(this.entlasserstdatum.getText()
                                                         .trim(),
                    this.entlassletztdatum.getText()
                                          .trim(),
                    this.entlassstunde.getText()
                                      .trim()
                            + this.entlassminute.getText()
                                                .trim(),
                    entlassafcombo.getSelectedIndex(), entlassartcombo.getSelectedIndex(),
                    entlassmitfahrgeld.isSelected(), entlassfahrgeld.getText(), entlasstpan.getText(),
        ...
        if (art == 3) {
            ...
            RVMeldung301 meldung301 = new RVMeldung301(art, id,
                    Integer.parseInt(SqlInfo.holeEinzelFeld(
                            "select count(*) from dtafall where nachrichtorg like '%PRC+ADMIN4%' AND rez_nr='"
                                    + this.reznummer + "'"))
                            + 1);
            int iart = -1;
            for (int i = 0; i < 3; i++) {
            ...
            long tage = DatFunk.TageDifferenz(this.vbbeginndatum.getText()
                                                                .trim(),
                    this.vbendedatum.getText()
                                    .trim())
                    + 1;
            // tage++;
            meldung301.doVerlaengerung(this.vbbeginndatum.getText()
                                                         .trim(),
                    this.vbendedatum.getText()
                                    .trim(),
                    iart, this.veditpan.getText()
                                       .trim(),
                    Long.toString(tage));
            doTabelleFuellen();
            return true;
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                try {
                    ...
                    if (!doRegle301(0, SqlInfo.holeEinzelFeld("select id from dta301 where pat_intern='" + patnummer
                            + "' and rez_nr='" + reznummer + "' and nachrichtentyp='1' LIMIT 1"))) {
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                try {
                    ...
                    if (!doRegle301(3, SqlInfo.holeEinzelFeld("select id from dta301 where pat_intern='" + patnummer
                            + "' and rez_nr='" + reznummer + "' and nachrichtentyp='1' LIMIT 1"))) {
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                try {
                    ...
                    if (!doRegle301(1, SqlInfo.holeEinzelFeld("select id from dta301 where pat_intern='" + patnummer
                            + "' and rez_nr='" + reznummer + "' and nachrichtentyp='1' LIMIT 1"))) {
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                try {
                    ...
                    if (!doRegle301(2, SqlInfo.holeEinzelFeld("select id from dta301 where pat_intern='" + patnummer
                            + "' and rez_nr='" + reznummer + "' and nachrichtentyp='1' LIMIT 1"))) {
    ...
    private boolean executeNachricht(int art) {
        ...
        int unterbart = 0;
        int verlart = 0;
        boolean keingrundbeginn = beginneditpan.getText()
                                               .trim()
                                               .equals("");
        ...
        if (art == 0) {
            ...
            if (aufnahmeart == 0) {
                meldung = "<html><font color='#ff0000' size=+2>Nachricht mit diesen Parametern erzeugen?</font><br><br>"
                        + "Narchittyp: <b>Aufnahmemitteilung" + "</b><br><br>Aufnahmedatum: <b>" + beginndatum.getText()
                        + "</b><br><br>Aufnahme Uhrzeit: <b>" + beginnstunde.getText() + ":" + beginnminute.getText()
                        + "</b><br><br>Fallart: <b>"
                        + (this.beginnahbcheck.isSelected() ? "AHB/AR Fall" : "medizinische Reha")
                        + (this.beginnahbcheck.isSelected()
                                ? "</b><br><br>KKH-Entlassdatum: <b>" + this.beginnahbentlass.getText()
                                : "<b>")
                        + "</b><br><br><b>Begründung bei Aufnahme i.d.R. nicht erforderlich</b>" + "</html>";
            } else if (aufnahmeart == 1) {
                meldung = "<html><font color='#ff0000' size=+2>Nachricht mit diesen Parametern erzeugen?</font><br><br>"
                        + "Narchittyp: <b>Rückstellung einer Aufnahme" + "</b><br><br>Rückstellung bis Datum: <b>"
                        + beginndatum.getText() + "</b><br><br><b>Rückstellung Uhrzeit nicht erforderlich"
                        + "</b><br><br>"
                        + (keingrundbeginn
                ...
                meldung = "<html><font color='#ff0000' size=+2>Nachricht mit diesen Parametern erzeugen?</font><br><br>"
                        + "Narchittyp: <b>Absage an den Kostenträger" + "</b><br><br>Absagedatum bis Datum: <b>"
                        + beginndatum.getText() + "</b><br><br><b>Absage Uhrzeit nicht erforderlich" + "</b><br><br>"
                        + (keingrundbeginn
        ...
        if (art == 1) {
            ...
            if (unterbart == 0) {
                meldung = meldung + "Narchittyp: <b>Beginn einer Unterbrechung melden"
                        ...
                        + "</b><br><br>Grund der Unterbrechung: " + ucombo.getSelectedItem()
                                                                          .toString()
                        + "</b><br><br><b>Erklärungstext nicht unbedingt erforderlich" + "</b></html>";
            }
            if (unterbart == 1) {
        ...
        if (art == 2) {
            ...
            if (!entlassnurfahrgeld.isSelected()) {
                meldung = meldung + "Narchittyp: <b>Entlassmitteilung" + "</b><br><br>Aufnahmetag: <b>"
                        + entlasserstdatum.getText() + "</b><br><br>Entlasstag: <b>" + entlassletztdatum.getText()
                        + "</b><br><br>Entlass-Uhrzeit: <b>" + entlassstunde.getText() + ":" + entlassminute.getText()
                        + "</b><br><br>Arbeitsfähigkeit: <b><font color='#ff0000'>" + entlassafcombo.getSelectedItem()
                                                                                                    .toString()
                        + "</font>" + "</b><br><br>Arbeitsform: <b>" + entlassartcombo.getSelectedItem()
                                                                                      .toString();
            } else {
        ...
        if (art == 3) {
            ...
            if (verlart == 0) {
                meldung = meldung + "Narchittyp: <b>Verlängerungsmitteilung"
                        ...
                        + Long.toString(tage) + " Tage)" + "</b><br><br><b>Erklärungstext nicht unbedingt erforderlich";
            } else {
                meldung = meldung + "Narchittyp: <b>Antrag auf Verlängerung"
                        + "</b><br><br>Erster Tag der Verlängerung: <b>" + vbbeginndatum.getText()
                        + "</b><br><br>Letzter Tag der Verlängerung: <b>" + vbendedatum.getText() + " ("
                        + Long.toString(tage) + " Tage)";
                if (veditpan.getText()
                            .trim()
                            .equals("")) {
                    meldung = meldung
                            + "</b><br><br><b><font color='#ff0000'>Achtung bei Absagen muß eine Begründung eingegeben werden</font>"
                            + "</b></html>";
                } else {
                    meldung = meldung + "</b><br><br><b>Medizinsiche Begründung wurde eingegeben" + "</b></html>";
                }
            }
        }
        ...
        if ((art == 3) && (aufnahmeart > 0) && (veditpan.getText()
                                                        .trim()
                                                        .equals(""))) {
            return false;
        }
