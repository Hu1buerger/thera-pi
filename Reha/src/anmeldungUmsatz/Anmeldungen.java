...
public class Anmeldungen extends JXPanel {
    ...
    private JXPanel getContent() {
        ...
        anmeldetbl.getColumn(4)
                  .setMinWidth(0);
        anmeldetbl.getColumn(4)
                  .setMaxWidth(0);
        anmeldetbl.getColumn(5)
                  .setMinWidth(0);
        anmeldetbl.getColumn(5)
                  .setMaxWidth(0);
        anmeldetbl.getColumn(5)
        ...
        this.content.add((kglab[0] = new JLabel("Physio")),
        ...
        this.content.add((kglab[3] = new JLabel("0,00")),
                cc.xyw(9, 8, 2, CellConstraints.RIGHT, CellConstraints.DEFAULT));
        this.content.add((kglab[4] = new JLabel("0")),
                cc.xyw(11, 8, 2, CellConstraints.RIGHT, CellConstraints.DEFAULT));

        this.content.add((malab[0] = new JLabel("Massage")),
        ...
        this.content.add((polab[3] = new JLabel("0,00")),
                cc.xyw(9, 18, 2, CellConstraints.RIGHT, CellConstraints.DEFAULT));
        this.content.add((polab[4] = new JLabel("0")),
        ...
        this.content.add(
                (this.buts[1] = ButtonTools.macheButton("komplette Rezeptdaten in OO-Calc übertragen", "calc", al)),
                    ...
                    new SwingWorker<Void, Void>() {
                        ...
                        protected Void doInBackground() throws Exception {
                            try {
                                ...
                                /*
                                 * setCursor(Reha.instance.wartenCursor); fortschrittDlg = new FortschrittDlg();
                                 * fortschrittDlg.setzeLabel("Ermittle Rezeptanzahl");
                                 * fortschrittDlg.setVisible(true); fortschrittDlg.toFront();
                                 */
                                doErmitteln();
                            } catch (Exception ex) {
                                ex.printStackTrace();
        ...
        tblmouse = new MouseListener() {
            ...
            public void mouseClicked(MouseEvent arg0) {
                ...
                if (clicks == 2) {
                    ...
                    String reznum = anmeldetbl.getValueAt(row, 1)
                                              .toString();
                    String pat_intern = anmeldetbl.getValueAt(row, 4)
                                                  .toString();
            ...
            protected Void doInBackground() throws Exception {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(null,
                            "Die Datumswerte des eingegebenen Zeitraumes sind nicht korrekt");
                    // beendeFortschritt();
                    return null;
                }
                ...
                Vector<Vector<String>> vec = SqlInfo.holeFelder(
                        "select * from verordn where datum >='" + dat1 + "' AND datum <='" + dat2 + "'");
                ...
                for (int i = 0; i < anzahl; i++) {
                    ...
                    vec2.clear();
                    try {
                        vec2.add(DatFunk.sDatInDeutsch(vec.get(i)
                                                          .get(22)
                                                          .trim()));
                    } catch (Exception ex) {
    ...
    private void doRechnen(Vector<String> vec) {
        int idisziplin = this.listdiszi.indexOf(vec.get(1)
                                                   .substring(0, 2));
        int anzahl;
            ...
            try {
                if (Integer.parseInt(vec.get(i + 8)) > 0) {
                    ...
                    if (i == 0) {
                        ...
                        anzahlMinuten[idisziplin] = anzahlMinuten[idisziplin]
                                + (Integer.parseInt(vec.get(47)) * anzahl);
                    }
                    bdpreis = BigDecimal.valueOf(Double.parseDouble(vec.get(i + 3)))
                                        .multiply(BigDecimal.valueOf(Double.parseDouble(vec.get(i + 18))));
                    bdgesamt = BigDecimal.valueOf(umsaetze[idisziplin])
                                         .add(bdpreis);
        ...
        if (vec.get(43)
            ...
            if (hbs[0] != null) {
                bdgesamt = BigDecimal.valueOf(umsaetze[idisziplin])
                                     .add(BigDecimal.valueOf((Double) hbs[0]));
                umsaetze[idisziplin] = bdgesamt.doubleValue();
            }
            if (hbs[1] != null) {
                bdgesamt = BigDecimal.valueOf(umsaetze[idisziplin])
                                     .add(BigDecimal.valueOf((Double) hbs[1]));
                umsaetze[idisziplin] = bdgesamt.doubleValue();
            }
            /*
             * //System.out.println("HB-Werte für Rezept: "+vec.get(1));
             * //System.out.println(hbs[0]); //System.out.println(hbs[1]);
             */
        }
