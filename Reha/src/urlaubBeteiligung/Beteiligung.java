...
public class Beteiligung extends JXPanel {
    ...
    String[] kopfzeile = { "Patient", "PG", "Pos1", "Pos2", "Pos3", "Pos4", "HB", "WG", "Preis1", "Preis2", "Preis3",
            "Preis4", "HB-Pr.", "WG-Pr.", "Summe", "A", "FK", "Min" };
    ...
    private void doTagAnalysieren(Vector<String> tag, int anzahltermine) {
        ...
        for (int i = 0; i < anzahltermine; i++) {
            ...
            progresstext1.setText("Auswerten Tag: " + DatFunk.sDatInDeutsch(tag.get(304)) + " Termin "
                    + Integer.toString(i + 1) + " von " + tag.get(300));
            inhistory = false;
            ohnezuordnung = false;
            reznum = tag.get((i * 5) + 1)
                        .trim();
            if ((reznum.indexOf("\\")) >= 0) {
                pos = reznum.indexOf("\\");
                    ...
                    if ((reznum.length() > 2)) {
                        if (("KGMAERLORHPO".contains(reznum.substring(0, 2)))) {
                            ...
                            if (ohnezuordnung) {
                                ...
                                if (rezvec.contains(vec.get(0)
                                                       .get(1))) {
                                    doKeineZuordnung(
                                            "an diesem Tag bereits erfaßt: " + tag.get((i * 5)) + "-" + vec.get(0)
                                                                                                           .get(1)
                                                                                                           .toString(),
                                            false);
                                    anzahlTagesBehandlungen++;
                                    ...
                                    try {
                                        doAnteilBerechnen(vec, inhistory, DatFunk.sDatInDeutsch(tag.get(304)),
                                                tag.get((i * 5)));
                                    ...
                                    rezvec.add(vec.get(0)
                                                  .get(1)
                                                  .toString());
                                    anzahlTagesBehandlungen++;
                                }
                                //// System.out.println("Tag "+DatFunk.sDatInDeutsch(tag.get(304))+" TerminNr
                                //// "+(i+1)+" = Rezeptnummer "+reznum);
                            }
                        } else {
                            doKeineZuordnung("ohne Zuordnung: " + tag.get((i * 5)), false);
                            anzahlTagesBehandlungen++;
    ...
    private void doAuswerten(int tage) {
        ...
        for (int i = 0; i < lang; i++) {
            // progressbar1.setValue(i);
            // progresstext1.setText("Auswerten Tag:
            // "+DatFunk.sDatInDeutsch(tagesvec.get(i).get(0).get(304))+
            // " Termin "+Integer.toString(i+1) +" von "+tagesvec.get(i).get(0).get(300));
            try {
                anzahlTagesBehandlungen = 0;
                doTagesKopf(DatFunk.sDatInDeutsch(tagesvec.get(i)
                                                          .get(0)
                                                          .get(304)));
                Integer[] summenStart = { calcrow + 1, 0 };
                summenPos.add(summenStart);
                doTagAnalysieren(tagesvec.get(i)
                                         .get(0),
                        Integer.parseInt(tagesvec.get(i)
                                                 .get(0)
                                                 .get(300)));
    ...
    private void doEinlesen(String start, String end, int differenz) {
        ...
        for (int i = 0; i <= differenz; i++) {
            ...
            cmd = "select * from flexkc where datum ='" + akttag + "'" + " AND behandler ='" + aktBehandlerNummer
                    + "BEHANDLER'";
     ...
     *****************************/
    private boolean doErmitteln()
            throws NoSuchElementException, WrappedTargetException, IndexOutOfBoundsException, UnknownPropertyException,
            PropertyVetoException, IllegalArgumentException, OfficeApplicationException, NOAException {
        if (jcmb.getValue()
                .toString()
                .trim()
                .equals("00")) {
        ...
        if (tfbeteil[0].getText()
            ...
            JOptionPane.showMessageDialog(null,
                    "Bei jeweils 0% Beteiligung gibt's eigentlich nichts was zu ermitteln wäre\nDeppen gibt's.....");
            tfbeteil[0].requestFocus();
            return false;
        }
        try {
            prozBehandl = Integer.parseInt(tfbeteil[0].getText()
                                                      .trim());
            prozHb = Integer.parseInt(tfbeteil[1].getText()
                                                 .trim());
            prozWeg = Integer.parseInt(tfbeteil[2].getText()
                                                  .trim());
        } catch (Exception ex) {
        ...
        long differenz = DatFunk.TageDifferenz(tfs[0].getText()
                                                     .trim(),
                tfs[1].getText()
                      .trim());
        if (differenz < 0) {
        ...
        al = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (cmd.equals("kollegen") && initok) {
                    aktBehandlerNummer = jcmb.getValue()
                                             .toString();
    ...
    private void doSpaltenJustieren() throws NoSuchElementException, WrappedTargetException, IndexOutOfBoundsException,
    ...
    private void doTagesKopf(String tag) throws IndexOutOfBoundsException, UnknownPropertyException,
    ...
    private void doTagesSumme(int tag) {
        try {
            ...
            String formel = "=sum(O" + Integer.toString(summenPos.get(tag)[0]) + ":O"
                    + Integer.toString(summenPos.get(tag)[1]) + ")";
    ...
    private void doEndsumme() {
        try {
            ...
            for (int i = 0; i < summenPos.size(); i++) {
                formula = formula + "O" + Integer.toString(summenPos.get(i)[1] + 1)
                        + (i < (summenPos.size() - 1) ? "+" : "");
                formula2 = formula2 + "R" + Integer.toString(summenPos.get(i)[1] + 1)
                        + (i < (summenPos.size() - 1) ? "+" : "");
            }
