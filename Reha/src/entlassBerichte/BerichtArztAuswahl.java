...
public class BerichtArztAuswahl extends JXPanel implements ActionListener, KeyListener, FocusListener {
    ...
    private JPanel getForm() {
        try {
            ...
            FormLayout lay2 = new FormLayout("fill:0:grow(0.33),150dlu,fill:0:grow(0.33),150dlu,fill:0:grow(0.33)",
                    "5dlu,p,5dlu");
        ...
        if (test.trim()
            ...
            for (int i = 0; i < arztid.length; i++) {
                ...
                Vector<Vector<String>> vec = SqlInfo.holeFelder(
                        "select nachname,vorname,strasse,ort,arztnum,bsnr,id from arzt where id = '" + arzt[1]
                                + "' LIMIT 1");
                if (vec.size() >= 1) {
    ...
    private void doArztAufnehmen() {
        ...
        // ArztAuswahl(JXFrame owner, String name,String[]
        // suchegleichnach,JRtaTextField[] elterntf,String arzt) {
        ...
        if (!tf[2].getText()
            ...
            if (vec.size() > 0) {
                ...
                if (!test.contains("@" + tf[2].getText()
                    ...
                    int frage = JOptionPane.showConfirmDialog(null, msg, "Wichtige Benutzeranfrage",
