...
public class SystemInit extends JXPanel implements TreeSelectionListener {
    ...
    public SystemInit(JSysteminitInternal sai) {
        ...
        addHeaderEntry(defaultTitel, "Mit der Systeminitialisierung....", // den ersten legen wir gleich an; Rest im
                                                                          // Hintergrund
        ...
        /****/
        // hier muß das add für die weitern Panels rein
        jSplitLR = UIFSplitPane.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT, jxLinks, jxRechts);
    ...
    /*
     * private static JXPanel titlePanel(){ SystemUtil.thisClass.jp = new RehaTP(0);
     * SystemUtil.thisClass.jp.getContentContainer().setLayout(new BorderLayout());
     * SystemUtil.thisClass.jtp = (JXPanel)
     * SystemUtil.thisClass.jp.getContentContainer();
     * SystemUtil.thisClass.jtp.setSize(new Dimension(200,200));
     * SystemUtil.thisClass.jtp.setName(SystemUtil.thisClass.dieserName);
     * SystemUtil.thisClass.jtp.setVisible(true); return SystemUtil.thisClass.jtp; }
     */
            ...
            if (util.equals("Software-Updateservice")) {
                if (!Reha.updatesBereit) {
                    ...
                    if (anfrage == JOptionPane.YES_OPTION) {
                        try {
                            Runtime.getRuntime()
                                   .exec("java -Djava.net.preferIPv4Stack=true -jar " + Path.Instance.getProghome()
                                           + "TheraPiUpdates.jar TheraPiStarten");
    ...
    public void valueChanged(TreeSelectionEvent e) {
        ...
        String[] split = path.toString()
                             .split(",");
        auswertenSysUtil(split[split.length - 1].replaceAll("\\[", "")
                                                .replaceAll("\\]", "")
                                                .trim());
    }
...
//private HashMap<String,ImageIcon> hicon = new HashMap();
    ...
    private void setHeader(String shm) {
        String key = shm;
        if (htitel.get(key) == null) {
            key = defaultTitel;
        } // Falls für eine Seite kein Hilfetext vorhanden ist, den Default-Text setzen
    ...
    private void addHeaderEntry(String key, String titel, String text, String icon) {
        String iconPath = Path.Instance.getProghome() + "icons/";
        if (icon == null) {
            iconPath = iconPath + "header-image.png";
        } else {
    ...
    private void headerInfos() {
        ...
        addHeaderEntry("Kalenderfarben", "Kalenderfarben definieren",
                ...
                null);
        /*********** Numero sex ***********/
        addHeaderEntry("Gruppentermine", "Gruppentermine definieren", "Gruppentermine ist was für den Spezialisten\n"
