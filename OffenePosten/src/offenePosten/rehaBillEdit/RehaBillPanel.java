  ...
  //  private OffenePosten offenePosten;
    ...
    private JXPanel getEdits() {
        ...
        try {
            ...
            jpan.validate();
            /*
             * lab = new JLabel("R-Datum alt:"); jpan.add(lab,cc.xy(2,26)); rdatumAlt = new
             * JLabel("  .  .    "); jpan.add(rdatumAlt,cc.xy(4,26));
             */
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    private void doZeileLoeschen() {
        int anfrage = JOptionPane.showConfirmDialog(null,
                "Sie sind im Begriff die´ausgewählte Position unwiederbringlich\n"
        ...
        if (anfrage == JOptionPane.YES_OPTION) {
            ...
            String id = tabmod.getValueAt(tab.convertRowIndexToModel(row), tabmod.getColumnCount() + ccount)
                              .toString()
                              .replace(".", "");
            ...
            rechneNeu(false);
            cmd = "update rliste set r_betrag='" + dcf.format(rechnungGesamt)
                                                      .replace(",", ".")
                    + "', r_offen='" + dcf.format(rechnungGesamt)
                                          .replace(",", ".")
    ...
    private void doInDbSpeichern() {
        ...
        cmd = "update rliste set r_betrag='" + dcf.format(rechnungGesamt)
                ...
                + "' where r_nummer='" + rnummerAlt.getText()
                                                   .trim()
                + "' LIMIT 1";
        // cmd = "update rliste set
        // r_betrag='"+dcf.format(rechnungGesamt).replace(",",".")+"' where
        // r_nummer='"+rnummerAlt.getText().trim()+"' LIMIT 1";
        // System.out.println(cmd);
        SqlInfo.sqlAusfuehren(cmd);
    ...
    private void bestueckeLabels(boolean mitwerten) {
        if (mitwerten) {
            String cmd = "select r_datum,r_betrag from rliste where r_nummer='" + tfs[0].getText()
                                                                                        .trim()
                    + "' LIMIT 1";
            ...
            if (vec.size() > 0) {
                ...
                rnummerAlt.setText(tfs[0].getText()
                                         .trim());
                for (int i = 0; i < 5; i++) {
                    tfs[i + 1].setText(tabmod.getValueAt(0, i)
                                             .toString());
                }
                tfs[6].setText(DatFunk.sDatInDeutsch(vec.get(0)
                                                        .get(0)));
            } else {
    ...
    private void rechneNeu(boolean inDBschreiben) {
        try {
            ...
            for (int i = 0; i < tabmod.getRowCount(); i++) {
                ...
                id = tabmod.getValueAt(i, tabmod.getColumnCount() + ccount)
                           .toString()
                           .replace(".", "");
                reiheAnzahl = BigDecimal.valueOf(Double.parseDouble(tabmod.getValueAt(i, 11)
                                                                          .toString()));
                reiheEinzel = BigDecimal.valueOf((Double) tabmod.getValueAt(i, 12));
                reiheGesamt = reiheAnzahl.multiply(reiheEinzel);
                reiheRezgeb = BigDecimal.valueOf(Double.parseDouble(tabmod.getValueAt(i, 16)
                                                                          .toString()));
                ...
                if (inDBschreiben) {
                    cmd = "update faktura set anzahl='" + Integer.toString(reiheAnzahl.intValue()) + "', preis='"
                            ...
                            + "', gesamt='" + dcf.format(reiheGesamt)
                                                 .replace(",", ".")
                            + "', netto='" + dcf.format(reiheNetto)
                                                .replace(",", ".")
                            + "', zzbetrag='" + dcf.format(reiheRezgeb)
                                                   .replace(",", ".")
                            + "', pauschale='" + dcf.format(reihePauschale)
                                                    .replace(",", ".")
                            + "' where id='" + id + "' LIMIT 1";
                    // System.out.println(cmd);
                    SqlInfo.sqlAusfuehren(cmd);
...
//                SqlInfo.sqlAusfuehren(cmd);
        ...
        try {
            ...
            if (fuerwen.equals("privat") && heilmittel) {
                ...
                try {
                    ...
                    int pg = -1;
                    try {
                        pg = Integer.parseInt(SqlInfo.holeEinzelFeld(
                                "select preisgruppe from lza where rez_nr='" + reznr + "' LIMIT 1"));
                    } catch (Exception ex) {
                        try {
                            pg = Integer.parseInt(SqlInfo.holeEinzelFeld(
                            ...
                            JOptionPane.showMessageDialog(null,
                                    "Das Rezept ist weder in der Historie noch im aktuellen Rezeptstamm vorhanden");
                            textDocument.close();
                            return;
                        }
                    }
    ...
    public void doPrivatWiederholungsdruck(Vector<Vector<String>> preisvec) {
        ...
        try {
            ...
            starteDokument(OffenePosten.progHome + "vorlagen/" + OffenePosten.aktIK + "/"
                    + OffenePosten.hmAbrechnung.get("hmpriformular") + ".Kopie.ott");
    ...
    private void constructRezHMap(String reznr) {
        ...
        try {
            hmAdrRDaten.put("<rnummer>", String.valueOf(rezdaten.get(0)
                                                                .get(0)));
            hmAdrRDaten.put("<rdatum>", DatFunk.sDatInDeutsch(String.valueOf(rezdaten.get(0)
                                                                                     .get(1))));
            String[] days = String.valueOf(rezdaten.get(0)
                                                   .get(2))
                                  .split("\n");
            String day = "";
    ...
    public void doBGEWiederholungsdruck(Vector<Vector<String>> preisvec) {
        ...
        try {
            ...
            // System.out.println(OffenePosten.hmRechnungPrivat);
            starteDokument(OffenePosten.progHome + "vorlagen/" + OffenePosten.aktIK + "/"
                    + OffenePosten.hmAbrechnung.get("hmbgeformular") + ".Kopie.ott");
            System.out.println(OffenePosten.progHome + "vorlagen/" + OffenePosten.aktIK + "/"
                    + OffenePosten.hmAbrechnung.get("hmbgeformular") + ".Kopie.ott");
            starteErsetzenBG();
            aktuellePosition = 0;
    ...
    public void doGKVWiederholungsdruck(String xreznr) {
        ...
        try {
            druckKopie = new AbrechnungDrucken(OffenePosten.progHome + "vorlagen/" + OffenePosten.aktIK + "/"
                    + OffenePosten.hmAbrechnung.get("hmgkvformular") + ".Kopie.ott", mittageChb.isSelected());
        } catch (Exception e) {
            e.printStackTrace();
        }

        String aktrezept = String.valueOf(xreznr);
        namenvornamen = String.valueOf(tabmod.getValueAt(0, 7)
                                             .toString()
                                             .trim());
        ...
        for (int i = 0; i < lang; i++) {
            if (!tabmod.getValueAt(i, 19)
                       ...
                       .equals(aktrezept)) {
                try {
                    druckKopie.setDaten(namenvornamen, status, aktrezept, position, anzahl, abrtage, einzelpreis, preis,
                            rezgeb, mitpauschale);
            ...
            if (placeholders[i].getDisplayText()
                ...
                placeholders[i].getTextRange()
                               .setText(hmAdresse.get("<pri7>"));
                                      ...
                                      .equals("<rnummer>")) {
                placeholders[i].getTextRange()
                               .setText(hmAdrRDaten.get("<rnummer>"));
            } else if (placeholders[i].getDisplayText()
                                      .toLowerCase()
                                      .equals("<rdatum>")) {
                placeholders[i].getTextRange()
                               .setText(hmAdrRDaten.get("<rdatum>"));
            } else if (placeholders[i].getDisplayText()
                                      .toLowerCase()
                                      .equals("<rtage>")) {
                placeholders[i].getTextRange()
                               .setText(hmAdrRDaten.get("<rtage>"));
            }
        }
    ...
    private void startePositionen() throws TextException {
        ...
        for (int i = 0; i < originalAnzahl.size(); i++) {
            ...
            textTable.getCell(3, aktuellePosition)
                     .getTextService()
                     .getText()
                     .setText(dcf.format(gesamtPreis.get(i)));
