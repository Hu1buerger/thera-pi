...
public class RehaUrlaubPanel extends JXPanel implements TableModelListener {
    ...
    String[] tooltips = { "Normaler Arbeitstag ", "voller Urlaubstag ", "voller Feiertag ", "voller Krankheitstag ",
            "halber Urlaubstag und halber Feiertag", "halber Urlaubstag und halber normaler Arbeitstag",
    ...
    String[][] tageart = { { "----", "----" }, { "UuUu", "UuUu" }, { "FfFf", "FfFf" }, { "KkKk", "KkKk" },
            { "UuFf", "UuFf" }, { "Uu--", "Uu--" }, { "Ff--", "Ff--" }, { "Kk--", "Kk--" }, { "K???", "K???" },
            { "U???", "U???" }, { "F???", "F???" } };
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                try {
                    ...
                    holeTabellen();
                    holeKalUser();
                    funkcombo[0].setSelectedItem(DatFunk.sHeute()
                                                        .substring(6));
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
    ...
    private void macheTagArtCombo() {
        /*
         * String[][] tageart = {
         * {"Normal","----"},{"Urlaub","UuUu"},{"Feiertag","FfFf"},{"Krank","KkKk"},
         * {"1/2Url.1/2Fei.","UuFf"},{"1/2Url.1/2Norm.","UuNn"},{"1/2Fei.1/2Norm.",
         * "FfNn"}, {"1/2Krank 1/2Norm.","KkNn"}};
         */
    ...
    private void holeTabellen() {
        ...
        for (int i = 0; i < dummy.size(); i++) {
            vectabellen.add(String.valueOf(dummy.get(i)
                                                .get(0)));
        }
    }
        ...
        al = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (cmd.equals("calc")) {
                    ...
                    new RehaUrlaubCalc(getInstance(), funkcombo[0].getSelectedItem()
                                                                  .toString()
                                                                  .trim(),
        ...
        try {
            if (funkcombo[0].getSelectedIndex() > 0) {
                if (usercombo.getSelectedIndex() > 0) {
                    if (vectabellen.indexOf("urlaub" + funkcombo[0].getSelectedItem()
                                                                   .toString()
                                                                   .trim()) >= 0) {
                        doSucheUserDaten(funkcombo[0].getSelectedItem()
                                                     .toString(),
                                this.vecKalZeile.get(usercombo.getSelectedIndex())
                                                .get(1));
                        aktuelleWochenStunden = Double.parseDouble(usercombo.getValueAt(2)
                                                                            .toString()
                                                                            .replace(",", "."));
                        ...
                        try {
                            ...
                            if (zuschlag.doubleValue() > 0.0d) {
                                zuschlag = zuschlag.divide(BigDecimal.valueOf(Double.parseDouble("5.00")), 2,
                                        BigDecimal.ROUND_HALF_UP);
                                halberzuschlag = zuschlag.divide(BigDecimal.valueOf(Double.parseDouble("2.00")), 2,
                                        BigDecimal.ROUND_HALF_UP);
                            }
                        } catch (Exception ex) {
                        ...
                        JOptionPane.showMessageDialog(null,
                                "Die Urlaubstabelle für das gewählte Kalenderjahr existiert nicht");
                ...
                if (gestartet) {
                    new SwingWorker<Void, Void>() {
                        ...
                        protected Void doInBackground() throws Exception {
                            JOptionPane.showMessageDialog(null,
                                    "Stellen Sie zuerste das entsprechende Kalenderjahr ein");
                            return null;
                        }
                    }.execute();
                } else {
    ...
    private JXPanel getTabPanel() {
        ...
        for (int i = 3; i < 17; i += 2) {
            tab.getColumn(i)
               .setCellEditor(new DefaultCellEditor(tagArt));
            tab.getColumn(i)
               .setCellRenderer(new MitteRenderer());
        }
        for (int i = 2; i < 16; i++) {
    ...
    private void direktSchreiben(int row, int col, String value) {
        ...
        String cmd = "update " + this.aktuellerUrlaubFile + " set berechnet='" + value + "' where id ='" + id
                + "' LIMIT 1";
    ...
    public Double getAktArbeitszeit() {
        if (usercombo.getSelectedIndex() > 0) {
            return Double.parseDouble(usercombo.getValueAt(2)
                                               .toString()
                                               .trim()
                                               .replace(",", "."));
        }
    ...
    private void doJahrAnlegen() {
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                 ...
                 * System.out.println("ersteWocheIstNull = "+ersteWocheIstNull); int x = 0; if(x
                 * == 0){ return null; }
                 */
                try {
                    String von_bis = "";
                    for (int behandler = 1; behandler < 100; behandler++) {
                        ...
                        /*
                         * try{ matchcode = elternTab.vecKalUser.get(behandler-1).get(1);
                         * }catch(Exception ex){ JOptionPane.showMessageDialog(
                         * null,"Fehler bei der Zuordnung des Behandlers mit Kalenderzeile = "+Integer.
                         * toString(behandler)+"!!"); }
                         */
                        try {
                            for (int beh = 0; beh < elternTab.vecKalUser.size(); beh++) {
                                if (elternTab.vecKalUser.get(beh)
                                                        .get(0)
                                                        .equals(Integer.toString(behandler))) {
                                    matchcode = elternTab.vecKalUser.get(beh)
                                                                    .get(1);
                                    break;
                                }
                            }
                            ...
                            JOptionPane.showMessageDialog(null,
                                    "Fehler bei der Zuordnung des Behandlers mit Kalenderzeile = "
                                            + Integer.toString(behandler) + "!!");
                        }
                        /*
                         * String matchcode = // hier muß die Korrektur rein ! //vecKalUser = SqlInfo.
                         * holeFelder("select kalzeile,matchcode from kollegen2 order by kalzeile");
                         * (behandler <= elternTab.vecKalUser.size() ?
                         * elternTab.vecKalUser.get(behandler-1).get(1) : "unbelegt");
                         */
                        for (int i = startWoche; i <= letzteWoche; i++) {
                            ...
                            if (durchlauf == 0 && ersteWocheIstNull) {
                                ...
                                kalenderWoche = Integer.toString(durchlauf);
                                von_bis = ersterTag + "-" + letzterTag;
                                //// System.out.println("startWoche = 1 in durchlauf == "+i+" / "+ersterTag+"
                                //// "+letzterTag);
                            } else {
                            ...
                            SqlInfo.sqlAusfuehren("insert into " + urlaubstabelle + " set kw='" + kalenderWoche
                                    ...
                                    + "kal_benutzer='" + matchcode + "', " + "jahr='" + Integer.parseInt(jahr) + "'");
                            /*
                    ...
                    holeTabellen();
                    JOptionPane.showMessageDialog(null,
                            "Urlaubstabelle für das Jahr " + jahr + " wurde erfolgreich angelegt!");
                } catch (Exception ex) {
                    ex.printStackTrace();
                    RehaUrlaub.thisFrame.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
                    JOptionPane.showMessageDialog(null, "Fehler in Funktion Tabelle für Urlaub anlegen");
    ...
    private void doSucheUserDaten(String jahr, String kalzeile) {
        ...
        Statement stmt = null;
        ResultSet rs = null;
        String mystmt = "select * from urlaub" + jahr + " where jahr='" + jahr + "' and kal_zeile='" + kalzeile
                + "' ORDER BY kw LIMIT 55";
    ...
    public BigDecimal ermittleTag(String tag, int kaluser) {
        String cmd = "select * from flexkc where datum='" + DatFunk.sDatInSQL(tag) + "' and behandler='"
                + (kaluser < 10 ? "0" + Integer.toString(kaluser) + "BEHANDLER"
                        : Integer.toString(kaluser) + "BEHANDLER")
                + "' LIMIT 1";
    ...
    private String allesInString(int row, int col) {
        ...
        } else if (feldNamen.get(col)
                            .get(1)
                            .startsWith("varchar(")) {
            ret = tabmod.getValueAt(row, col)
                        .toString();
        } else if (feldNamen.get(col)
                            .get(1)
                            .startsWith("enum(")) {
    ...
    private void doErmitteln() {
        ...
        for (int i = 0; i < rows.length; i++) {
            erster = tab.getValueAt(rows[i], 1)
                        .toString()
                        .split("-")[0];
            id = (Integer) tabmod.getValueAt(rows[i], tabmod.getColumnCount() - 1);
            kalenderWerte.clear();
            kalenderWerte.trimToSize();
            kalenderWerte = SqlInfo.holeFelder(ermittleWoche(erster, Integer.parseInt(usercombo.getValueAt(1)
                                                                                               .toString())));
    ...
    private void analysiereWoche(Vector<Vector<String>> termine, int id, int row) {
        ...
        // Wenn die Woche bereits berechnet wurde evtl. nachfragen ob neuberechnung
        // ja/nein
        // b*5 = Name
        // (b*5)+1 Nummer
        // (b*5)+2 Start
        ...
        for (int i = 0; i < termine.size(); i++) {
            ...
            bloecke = Integer.parseInt(termine.get(i)
                                              .get(termine.get(i)
                                                          .size()
                                                      - 6));
            krank = false;
            ...
            for (int b = 0; b < bloecke; b++) {
                ...
                if (!termine.get(i)
                            .get((b * 5) + 1)
                            .equalsIgnoreCase("@FREI")) {

                    // Minuten aufaddieren
                    tagdauer = tagdauer.add(BigDecimal.valueOf(Double.parseDouble(termine.get(i)
                                                                                         .get((b * 5) + 3)
                            + ".000")));
                    ...
                    if (termine.get(i)
                            ...
                            && termine.get(i)
                                      ...
                                      .trim()
                                      .equals("")) {
                        ausfalldauer = ausfalldauer.add(BigDecimal.valueOf(Double.parseDouble(termine.get(i)
                                                                                                     .get((b * 5) + 3)
                                + ".000")));
                    }
                }
        ...
        ausfalldauer = ausfalldauer.divide(BigDecimal.valueOf(Double.parseDouble("60.00")), 2,
                BigDecimal.ROUND_HALF_UP);
        ...
        tabmod.setValueAt(Double.parseDouble(usercombo.getValueAt(2)
                                                      .toString()
                                                      .replace(",", ".")),
                row, 17);
    ...
    private BigDecimal analysiereTag(Vector<Vector<String>> termine) {
        ...
        bloecke = Integer.parseInt(termine.get(0)
                                          .get(termine.get(0)
                                                      .size()
                                                  - 6));
        ...
        for (int b = 0; b < bloecke; b++) {
            ...
            if (!termine.get(0)
                ...
                // Minuten aufaddieren
                tagdauer = tagdauer.add(BigDecimal.valueOf(Double.parseDouble(termine.get(0)
                                                                                     .get((b * 5) + 3)
                        + ".000")));
                ...
                if (termine.get(0)
                        ...
                        && termine.get(0)
                                  ...
                                  .trim()
                                  .equals("")) {
                    ausfalldauer = ausfalldauer.add(BigDecimal.valueOf(Double.parseDouble(termine.get(0)
                                                                                                 .get((b * 5) + 3)
                            + ".000")));
                }
    ...
    public void rechneTagArt(int row, int col) {
        ...
        if (wert.equals("----")) {
            istarbeit = ermittleTag(tag, Integer.parseInt(usercombo.getValueAt(1)
                                                                   .toString()));
            ...
            istarbeit = ermittleTag(tag, Integer.parseInt(usercombo.getValueAt(1)
                                                                   .toString()));
            tabmod.setValueAt(istarbeit.add(halberzuschlag)
                                       .doubleValue(),
                    row, col - 1);
